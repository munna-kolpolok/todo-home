...
@la_access("videos", "create")
	...
	<a href="{{ url(config('laraadmin.adminRoute') . '/videos/create') }}" class="btn btn-success btn-sm pull-right"> <i class="fa fa-plus" aria-hidden="true"></i> @lang("messages.Add Video")</a>
@endla_access
@endsection
	...
	<div class="box-body">
		...
		<tbody>
			...
			<tr>
				...
				<td>{{$value->video_category->name}}</td>
				<td><img src="{{asset($value->image)}}" alt="image" width="60px" height="60px"></td>
				<td><a href="{{$value->video_link}}" target="_blank">{{$value->video_link}}</a></td>

				<td>
					<a href="{{ url(config('laraadmin.adminRoute') .'/videos/'.$value->id) }}" class="btn btn-success btn-xs" data-toggle="tooltip" title="Details" style="display:inline;padding:2px 5px 3px 5px;"><i class="fa fa-eye"></i></a>
					@la_access("videos", "edit")
					<a href="{{url(config('laraadmin.adminRoute').'/videos/'.$value->id.'/edit')}}"
					...
					{{Form::open(['route' => [config('laraadmin.adminRoute') . '.videos.destroy', $value->id], 'method' => 'delete', 'style'=>'display:inline'])}}
