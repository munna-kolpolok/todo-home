...
@section("contentheader_description", "Sectors Order Listing")
@section("section", trans("messages.Sectors"))
@section("sub_section", trans("messages.Listing"))
@section("htmlheader_title", "Sectors Order Listing")
        ...
        <div class="box-body">
            <table id="example1" class="table table-bordered">
                ...
                @foreach($sectors as $key=>$sector)
                    <tr class="row1" data-id="{{ $sector->id }}">
                        <td><i class="fa fa-sort fa-2x" aria-hidden="true"></i></td>
                        <td>{{$sector->serial_no}}</td>
                        <td>{{$sector->website->name}}</td>
                        <td>{{$sector->name}}</td>
                    </tr>
                @endforeach
                </tbody>
    ...
    $(function () {
        ...
        function sendOrderToServer() {
            ...
            $.ajax({
                type: "POST",
                dataType: "json",
                url: "{{ url('admin/sectors/sector-order-update') }}",
                data: {
                    order: order,
                    _token: '{{csrf_token()}}'
                },
