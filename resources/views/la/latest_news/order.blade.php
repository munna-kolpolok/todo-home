...
@section("contentheader_description", "Latest News Order Listing")
@section("section", trans("messages.Latest News"))
@section("sub_section", trans("messages.Listing"))
@section("htmlheader_title", "Latest News Order Listing")
        ...
        <div class="box-body">
            <table id="example1" class="table table-bordered">
                ...
                @foreach($latestNews as $key=>$news)
                    <tr class="row1" data-id="{{ $news->id }}">
                        <td><i class="fa fa-sort fa-2x" aria-hidden="true"></i></td>
                        <td>{{$news->serial_no}}</td>
                        <td>{{$news->website->name}}</td>
                        <td>{{$news->news}}</td>
                    </tr>
                @endforeach
                </tbody>
    ...
    $(function () {
        ...
        function sendOrderToServer() {
            ...
            $.ajax({
                type: "POST",
                dataType: "json",
                url: "{{ url('admin/latest_news/news-order-update') }}",
                data: {
                    order: order,
                    _token: '{{csrf_token()}}'
                },
