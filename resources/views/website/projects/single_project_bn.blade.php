                            ...
                            <label class="radio-inline">
                                <input name="amount" checked
                                       ...
                                       type="radio" class="usd_radio_project_form"  onclick="usd_project_form_load()"/><span
                                        class="in-currency">$</span> {{\App\Helpers\CommonHelper::en2bnNumber(number_format(ceil($donation_amount->amount/$currency_usd->tk_convert_amount)))}}
                                ({{$donation_amount->bn_description or null}})
                            </label>
                        </div>
                    @endforeach
                                    ...
                                    <label class="radio-inline">
                                        <input name="amount" checked
                                               ...
                                               type="radio" class="bdt_radio_project_form"  onclick="bdt_project_form_load()"/><span
                                                class="bd-currency">৳</span> {{\App\Helpers\CommonHelper::en2bnNumber(\App\Helpers\CommonHelper::bd_money_format_wod($donation_amount->amount))}}
                                        ({{$donation_amount->bn_description or null}})
                                    </label>
                                </div>
                            @endif
                                ...
                                <div class="event-info">
                                    <ul>
                                        ...
                                        @if(!empty($project->bn_achievement))
                                            <li>
                                                <i class="fa fa-fax"></i> {{ $project->bn_achievement or null}}
                                            </li>
                                        @endif
                                    </ul>
                                </div>
            ...
            <div class="row">
                <div class="col col-md-12">
                    <div class="subscribe">
                        <input type="hidden" id="project-id-{{$project->id}}"
                               value="{{$project->id}}">
                        <h4>{{$project->bn_donation_title or null}}</h4>
                        <a href="#" class="btn theme-btn donate-project" data-toggle="modal"
                           data-target="#donate-project-modal">@lang('messages.Donate now')</a>
                    </div>
                </div>
...
<script>
    ...
    {
        $("#custom_bdt_project_form").attr("placeholder", "<?php echo Lang::get('messages.Minimum donation amount is').' '. \App\Helpers\CommonHelper::en2bnNumber($currency_bdt->min_donate_amount).' '. Lang::get('messages.Taka')?>");
    ...
    {
        $("#custom_usd_project_form").attr("placeholder", "<?php echo Lang::get('messages.Minimum donation amount is').' '. \App\Helpers\CommonHelper::en2bnNumber($currency_usd->min_donate_amount).' '. Lang::get('messages.Dollar')?>");
