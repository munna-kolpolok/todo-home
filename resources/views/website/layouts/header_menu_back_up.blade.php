                            ...
                            @foreach($projects as $project)
                                @if($project->type==1)
                                    @if(empty($project->parent))
                                        ...
                                        @if(count($childs)==0)
                                            <li>
                                                <a href="{{ url('projects/'.str_replace(' ', '_', $project->name)) }}">{{ $project->name or null}}</a>
                                            </li>
                                        @else
                                            <li class="sub-sub-menu">
                                                <a href="{{ url('projects/'.$project->id) }}">{{ $project->name or null}}</a>
                                                <ul>
                                                    @foreach($childs as $child)
                                                        <li>
                                                            <a href="{{url('projects/'.$child->id)}}">{{ $child->name or null}}</a>
                                                        </li>
                                                    @endforeach
                                                </ul>
                                            </li>
